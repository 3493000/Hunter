...
namespace Hunter
{
    ...
    {
        //==========LoopScrollRect==========
        public delegate void CellPrefabDelegate(int index, out string prefabName, out GameObject prefab);
        public delegate void CellUpdateDelegate(Transform root, int index);
        [SerializeField]
        private GameObject m_PrefabGameObject;

        [HideInInspector]
        public CellPrefabDelegate cellPrefabDelegate = null;
        [HideInInspector]
        public CellUpdateDelegate cellUpdateDelegate = null;
        ...
        private GameObjectPoolGroup m_PoolGroup = null;
        ...
        [SerializeField]
        protected eDirection m_Direction = eDirection.kVertical;
        ...
        {
            if (cellUpdateDelegate == null)
            {
                return;
            }

            cellUpdateDelegate(go, idx);
        ...
        private void ReturnObjectAndSendMessage(Transform go)
        {
            ...
            m_PoolGroup.Recycle(go.gameObject);
        }
        ...
        public Transform GetCellAtIndex(int index)
        {
            ...
            return content.GetChild(childIndex);
        }
        ...
        public void RefillCells(int startIdx = 0)
        {
            if (Application.isPlaying)
            {
                ...
                if (cellPrefabDelegate != null/* && prefabCountFunc != null*/)
                {
        ...
        private void GetCellInfo(int itemIdx, out string poolName, out GameObject prefab)
        {
            if (cellPrefabDelegate == null)
            {
                poolName = "abs";
                prefab = m_PrefabGameObject;
                return;
            }

            cellPrefabDelegate(itemIdx, out poolName, out prefab);
        }
        ...
        private float m_CacheScrollBarValue = -1;
        private void UpdateScrollbars(Vector2 offset)
        {
            if (m_HorizontalScrollbar)
			{
                ...
                m_CacheScrollBarValue = horizontalNormalizedPosition;

                if (m_CacheScrollBarValue > 1)
                {
                    m_CacheScrollBarValue = 1;
                }
                else if (m_CacheScrollBarValue < 0)
                {
                    m_CacheScrollBarValue = 0;
                }

                m_HorizontalScrollbar.value = m_CacheScrollBarValue;
            }
            ...
            if (m_VerticalScrollbar)
			{
                ...
                m_CacheScrollBarValue = verticalNormalizedPosition;
                ...
                m_VerticalScrollbar.value = m_CacheScrollBarValue;
            }
        }
        ...
        {
            if (m_CacheScrollBarValue == value)
            {
                return;
            }
